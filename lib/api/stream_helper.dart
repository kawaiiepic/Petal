...
class StreamApi {
      ...
      try {
        ...
        if (episode != null) {
          ...
          allStreams.addAll(
            mappedStreams.where(
              (s) =>
                  s.season == episode.season && s.episode == episode.episode ||
                  s.name.toUpperCase().contains(tag) ||
                  s.external,
            ),
          );
        } else {
  ...
  static Future<List<CatalogItem>> searchCatalogItems(
    String query,
    List<Addon> addons,
  ) async {
    ...
    for (final addon in addons) {
      ...
      final List<Catalog> catalogs =
          (addon.manifest?["catalogs"] as List<dynamic>?)
              ?.map((c) => Catalog.fromJson(c as Map<String, dynamic>))
              .toList() ??
          [];
  ...
  static Future<CatalogItem?> fetchCatalogItemById(
    String id,
    String type, {
    String baseUrl = 'https://v3-cinemeta.strem.io/meta',
  }) async {
  ...
  static Future<CatalogItem?> fetchCatalogItem(
    CatalogItem item, {
    String baseUrl = 'https://v3-cinemeta.strem.io/meta',
  }) async {
